document.addEventListener("DOMContentLoaded", function () {
    ...
    let rotatedFiles = new Map(); // key: filename, value: { blob, name, angle, img }
    ...
    function drawRotatedImage(img, angle, callback) {
      ...
      tempCanvas.toBlob(blob => {
        callback(blob, tempCanvas.toDataURL("image/png"));
      }, "image/png");
    }
    ...
    function updatePreview(fileName, angle) {
      ...
      drawRotatedImage(entry.img, angle, (blob, dataUrl) => {
    ...
    function createPreview(file, img, angle = 0) {
      const rotatedName = file.name.replace(/\.[^/.]+$/, "") + "_rotated.png";
      ...
      drawRotatedImage(img, angle, (blob, dataUrl) => {
        imageEl.src = dataUrl;
        downloadLink.href = dataUrl;
